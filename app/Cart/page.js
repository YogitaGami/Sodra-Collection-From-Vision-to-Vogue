...
import { updatePriceDetails, selectPriceDetails } from "@/redux/priceDetails/priceDetailsSlice";
...
const Cart = () => {
  ...
  const updateRentalDays = (itemId, newDays) => {
    ...
    // Update Redux state
    dispatch(addItem({ ...cart.find((item) => item.id === itemId), selectedDays: newDays }));
  };
  ...
  const handleToggleSelectedItems = (event) => {
    ...
    setCart(
      cart.map((item) => ({ ...item, checked: isChecked }))
    );
  };

  const handleRemoveCheckedItems = () => {
    const checkedItemIds = cart.filter(
      (item) => item.checked
  ...
  return (
    <div className="py-20 mx-auto flex justify-center gap-3">
      <div className="cartitems w-[40%]">
        ...
        <div className="flex justify-between items-center mt-12">
          <div className="flex gap-2 items-center">
            <input
              ...
              checked={priceDetails.selectedItemQuantity === totalQuantity}
              onChange={handleToggleSelectedItems}
            />
            <div className="font-bold text-lg">
              {priceDetails.selectedItemQuantity}/{totalQuantity}
            </div>
          </div>
          <div className="flex ">
            <button onClick={handleRemoveCheckedItems} className="px-6 py-2">
              Remove
            </button>
        ...
        {cart.map((item) => {
          ...
          return (
            ...
            >
              <div>
                <div className="!relative w-32 h-44 border-[1px] border-[#54b7fa]">
                  <input
                    type="checkbox"
              ...
              <CartItems item={item} onUpdateDays={updateRentalDays} />
