...
const Login = () => {
  ...
  useEffect(() => {
    document.title = "Login | Sodra";

    const metaDescription = document.querySelector("meta[name='description']");
    if (metaDescription) {
      metaDescription.setAttribute(
        "content",
        "Login to Sodra to continue shopping for your favorite fashion items."
      );
    }
    if (session) {
      // Retrieve the previous page from localStorage
      const prevPage = localStorage.getItem("prevPage") || "/Dresses";
      localStorage.removeItem("prevPage"); // Clear it after redirecting
  ...
  const handleEmailLogin = async () => {
    ...
    try {
      const res = await signIn("email", {
        email,
        redirect: false, // so it doesn’t redirect on success/failure
      });
    ...
    <>
      ...
      <div className="flex justify-center items-center social-login-btn">
        <div className="flex flex-col gap-8 min-h-screen">
          <button className="flex items-center bg-white border border-pink-300 rounded-lg shadow-md max-w-sm px-14 py-4 text-sm font-medium text-gray-800 hover:bg-pink-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500">
            <svg
          ...
          <button
            ...
            className="flex items-center bg-white border border-pink-300 rounded-lg shadow-md max-w-sm px-14 py-4 text-sm font-medium text-gray-800 hover:bg-pink-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500"
          >
            <svg
              className="h-6 w-6 mr-2"
              xmlns="http://www.w3.org/2000/svg"
          ...
          <div className="text-center text-pink-400 mb-4">or</div>
          ...
          {emailSent ? (
            ...
            <div className="flex flex-col gap-3">
              <input
                ...
                className="bg-white border border-pink-300 rounded-lg shadow-md max-w-sm px-14 py-4 text-sm font-medium text-gray-800 hover:bg-pink-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500"
                value={email}
              ...
              <button
                ...
                disabled={loading}
                className="w-full border bg-pink-500 text-black font-semibold shadow-md mt-4 py-4 px-4 rounded-lg hover:bg-pink-600 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-700"
              >
